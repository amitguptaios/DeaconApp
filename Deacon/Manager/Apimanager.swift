...
class WebServices: NSObject {
    ...
    class func requestApiWithDictParam<T:Decodable>(url:String,requestType:RequestType,params:[String : Any],imageData:[Data?] = [],imageType:[ImageTpe?] = [],imageParameter:String = "",modalType:T.Type, completion : @escaping (_ response : T?, _ message: String?, _ success : Bool)-> ()) {
    ...
    //MARK :- Post
    class func postWebService(urlString: String,requestType:RequestType, params: [String : Any],imageData:[Data?],imageType:[ImageTpe?],imageParameter:String, completion : @escaping (_ response : Data?, _ message: String?, _ success : Bool)-> Void) {
    ...
    class func alamofireFunction(urlString : String, method : Alamofire.HTTPMethod, paramters : [String : Any],imageData:[Data?],imageType:[ImageTpe?],imageParameter:String, completion : @escaping (_ response : Data?, _ message: String?, _ success : Bool)-> Void){
                ...
                for index  in 0..<imageData.count{
                    if let data = imageData[index]{
                        if let getImageType = imageType[index]{
                            ...
                            multipartFormData.append(imageData[index] ?? Data(), withName: "\(imageParameter)\(index+1)")
                        }
                    }
                }
            },
